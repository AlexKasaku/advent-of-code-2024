...
const parseInput = (): Data => {
  ...
  return {
    rules: groups[0].split('\n').map((line) => {
      const x = line.split('|')
      return { before: Number.parseInt(x[0]), after: Number.parseInt(x[1]) }
    }),
    pagesSets: groups[1].split('\n').map((line) => line.split(',').map((x) => Number.parseInt(x)))
  }
}
...
const findSets = (data: Data, valid: boolean): Pages[] => {
  ...
  return data.pagesSets.filter((set) => {
    ...
    for (let i = 0; i < set.length; i++) {
      ...
      if (!beforeRulesMap.has(current)) { continue }
      ...
      for (let j = 0; j < remainder.length; j++) {
        ...
        if (beforeRulesMap.get(current)!.includes(toTest)) { return !valid }
      }
    }
...
const findNextValidPageIndex = (rulesMap: RulesMap, allPages: number[], candidates: number[], precedingPages: number[]) => {
  ...
  for (let index = 0; index < candidates.length; index++) {
    ...
    if (pagesThatMustBeBefore.filter((x) => allPages.includes(x) && !precedingPages.includes(x)).length === 0) { return index }
  }
